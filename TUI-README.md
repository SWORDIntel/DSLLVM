...
### Configuration File
The TUI uses a configuration file `.dsllvm-config` for sensitive settings:
...
**Note**: The `.dsllvm-config` file is automatically `.gitignore`'d to prevent credential exposure.
...
- **OS Recognition**: Supports Ubuntu, Debian, CentOS, RHEL, Fedora, Alpine
- **Target Triple Generation**: Creates proper LLVM target triples for cross-compilation
- **CPU Core Detection**: Uses all available CPU cores for parallel builds
...
sshpass -p '261505' ssh -p 22 dsmil@38.102.85.235

# With custom port
sshpass -p '261505' ssh -p 2222 dsmil@your-host.com
```
...
export SSH_PASS="custom_password"
```
