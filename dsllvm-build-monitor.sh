...
# Logging
LOG_FILE="/var/log/dsllvm-build-monitor.log"
...
monitor_builds() {
    ...
    while true; do
        ...
        local ninja_pids
        ninja_pids=$(pgrep -f "ninja.*DSLLVM\|ninja.*dsllvm" 2>/dev/null || true)
...
case "${1:-monitor}" in
    ...
    status)
        ...
        local active_builds
        active_builds=$(pgrep -f "ninja.*DSLLVM\|ninja.*dsllvm" 2>/dev/null | wc -l)
