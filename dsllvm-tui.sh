...
# - Remote host management with SSH password auth
...
check_dependencies() {
    ...
    for cmd in dialog sshpass ssh curl wget git; do
    ...
    if [[ ${#missing_deps[@]} -gt 0 ]]; then
        ...
        echo -e "${YELLOW}Install with: sudo apt-get install dialog sshpass openssh-client curl wget git${NC}"
        return 1
    fi
...
github_api_call() {
    ...
    if [[ -z "$GITHUB_PAT" ]]; then
        echo "GitHub PAT not configured. Please set GITHUB_PAT in $CONFIG_FILE"
        return 1
    fi
...
start_remote_build() {
    ...
    if ! remote_exec "$host" "$user" "$port" "mkdir -p $REMOTE_BUILD_DIR"; then
        echo -e "${RED}Failed to create build directory${NC}"
        return 1
    fi

    # Clone/update repository
    if remote_exec "$host" "$user" "$port" "test -d $REMOTE_BUILD_DIR/dsllvm-src" >/dev/null; then
        echo "Updating existing repository..."
        if ! remote_exec "$host" "$user" "$port" "cd $REMOTE_BUILD_DIR/dsllvm-src && git fetch origin && git checkout $branch && git pull origin $branch"; then
            echo -e "${RED}Failed to update repository${NC}"
            return 1
        fi
    else
        echo "Cloning repository..."
        if ! remote_exec "$host" "$user" "$port" "cd $REMOTE_BUILD_DIR && git clone --branch $branch $REPO_URL dsllvm-src"; then
    ...
    if ! remote_exec "$host" "$user" "$port" "cd $REMOTE_BUILD_DIR/dsllvm-src && nohup ./install-dsllvm.sh --prefix $REMOTE_BUILD_DIR/install --thermal-max 85 --thermal-critical 95 --clean --resume --skip-install > $REMOTE_BUILD_DIR/build.log 2>&1 & echo \$! > $REMOTE_BUILD_DIR/build.pid"; then
...
download_repository() {
    ...
    if [[ -n "$branch" ]]; then
        ...
        if start_remote_build "$REMOTE_HOST" "$REMOTE_USER" "$REMOTE_PORT" "$branch" 2>/dev/null; then
...
view_system_info() {
    ...
    local info
    info=$(get_remote_info "$REMOTE_HOST" "$REMOTE_USER" "$REMOTE_PORT")
...
check_github_access() {
    ...
    if [[ -n "$GITHUB_PAT" ]]; then
        auth_status="PAT configured ✓"
        clone_speed="Fast authenticated cloning available"
    else
        auth_status="No PAT configured ⚠"
        clone_speed="Slow/rate limited cloning"
    fi

    if check_repo_access; then
        dialog --msgbox "GitHub access confirmed!\n\nRepository: SWORDIntel/DSLLVM\nAuthentication: $auth_status\n\n$clone_speed" 10 60
    else
        dialog --msgbox "GitHub access failed!\n\nCheck PAT configuration in $CONFIG_FILE\n\nAuthentication: $auth_status" 10 60
    fi
}
...
show_configuration() {
    ...
    config+="GitHub PAT: ${GITHUB_PAT:+Configured (hidden)}"
